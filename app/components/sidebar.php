  ...
  <ul class="mt-4 flex-1">
    ...
    foreach ($this->model->list('id,attributes,icon,title,subtitle,url', 'permissions', "and type = 'menu' ORDER BY sort,title") as $r) {
      if (in_array($r->id, json_decode($user->permissions, true)
        ...
        if ($r->title != $current_title) {
          ...
          <?php } ?>
            <li class='mb-1 group' <?php echo ($r->title != $menu) ? "x-data='{ dropdown: false }'" : "x-data='{ dropdown: true }'"; ?>>
              <a
                  href='#'
                  class='flex items-center py-2 px-4 text-black <?php echo ($r->title != $menu) ? "hover:bg-gray-900" : "bg-gray-900 text-white"; ?> hover:text-gray-100 rounded-md'
                  @click='dropdown = !dropdown'
                  :class="dropdown ? 'bg-gray-900 text-white' : ''"
              >
                  <?php echo $r->icon ?> 
        ...
        <li class='mb-4'>
          <a href="<?php echo $r->url ?>" <?= $r->attributes ?> class="cursor-pointer text-sm flex items-center <?php echo ($r->url != $active) ? "hover:text-gray-700" : "text-gray-700"; ?>  before:contents-[''] before:w-1 before:h-1 before:rounded-full before:bg-gray-300 before:mr-3">
