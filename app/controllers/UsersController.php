  ...
  public function Data(){
    ...
    if (in_array(1, $permissions)) {
      ...
      foreach ($this->model->list("id,createdAt as date,username as name,email,if(status=1,'Activo','Inactivo') as status","users") as $k => $v) {
        $b1 = ($v->status != 'Activo')
        ? "<a hx-get='?c=Users&a=Status&id=$v->id&status=1' hx-swap = 'outerHTML' class='mx-3 text-teal-900 hover:text-teal-700 cursor-pointer float-right'><i class='ri-toggle-line text-2xl'></i></a>" 
        : "<a hx-get='?c=Users&a=Status&id=$v->id&status=0' hx-swap = 'outerHTML' class='mx-3 text-teal-900 hover:text-teal-700 cursor-pointer float-right'><i class='ri-toggle-fill text-2xl'></i></a>";
        $b2 = "<a hx-get='?c=Users&a=Profile&id=$v->id' hx-target='#myModal' @click='showModal = true' class='text-teal-900 hover:text-teal-700 cursor-pointer float-right mx-3'><i class='ri-edit-2-line text-2xl'></i></a>";
  ...
  public function Status(){
    ...
    if (in_array(1, $permissions)) {
      ...
      echo ($_REQUEST['status'] == 1)
        ? "<a hx-get='?c=Users&a=Status&id=$id&status=0' hx-swap = 'outerHTML' class='block mx-3 text-teal-900 hover:text-teal-700 cursor-pointer float-right'><i class='ri-toggle-fill text-2xl'></i></a>"
        : "<a hx-get='?c=Users&a=Status&id=$id&status=1' hx-swap = 'outerHTML' class='block mx-3 text-teal-900 hover:text-teal-700 cursor-pointer float-right'><i class='ri-toggle-line text-2xl'></i></a>";
    } else {
      $this->model->redirect();
    }
  }
